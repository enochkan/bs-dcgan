...
class DCGAN(object):
  def __init__(self, sess, input_height=108, input_width=108, crop=True,
    ...
    # if not os.path.isfile('/path/to/csv'):
      ...
      for idx in xrange(0, int(batch_idxs)):
        if config.dataset == 'mnist':
          ...
          try:
            ...
            print ("failed on batch id: "+ str(batch_idxs))
        ...
        with open('./logs/loss_log.csv', 'a', newline='') as f:
          writer = csv.writer(f)
          writer.writerow([errD_fake+errD_real,errG,counter])
  ...
  def discriminator(self, image, y=None, reuse=False):
    with tf.variable_scope("discriminator") as scope:
      ...
      if not self.y_dim:
        h0 = tf.nn.selu(conv2d(image, self.df_dim, name='d_h0_conv'))
        h1 = tf.nn.selu(self.d_bn1(conv2d(h0, self.df_dim*2, name='d_h1_conv')))
        h2 = tf.nn.selu(self.d_bn2(conv2d(h1, self.df_dim*4, name='d_h2_conv')))
        h3 = tf.nn.selu(self.d_bn3(conv2d(h2, self.df_dim*8, name='d_h3_conv')))
        h4 = tf.nn.selu(self.d_bn4(conv2d(h3, self.df_dim*16, name='d_h4_conv')))
        ...
        h0 = tf.nn.selu(conv2d(x, self.c_dim + self.y_dim, name='d_h0_conv'))
        h0 = conv_cond_concat(h0, yb)

        h1 = tf.nn.selu(self.d_bn1(conv2d(h0, self.df_dim + self.y_dim, name='d_h1_conv')))
        h1 = tf.reshape(h1, [self.batch_size, -1])      
        h1 = concat([h1, y], 1)
        
        h2 = tf.nn.selu(self.d_bn2(linear(h1, self.dfc_dim, 'd_h2_lin')))
        h2 = concat([h2, y], 1)
  ...
  def generator(self, z, y=None):
    with tf.variable_scope("generator") as scope:
      if not self.y_dim:
        ...
        h1 = tf.nn.selu(self.g_bn1(
            linear(h0, self.gf_dim*2*s_h4*s_w4, 'g_h1_lin')))
        h1 = tf.reshape(h1, [self.batch_size, s_h4, s_w4, self.gf_dim * 2])

        h1 = conv_cond_concat(h1, yb)

        h2 = tf.nn.selu(self.g_bn2(deconv2d(h1,
  ...
  def sampler(self, z, y=None):
    with tf.variable_scope("generator") as scope:
      ...
      if not self.y_dim:
        ...
        h0 = tf.nn.selu(self.g_bn0(linear(z, self.gfc_dim, 'g_h0_lin'), train=False))
        h0 = concat([h0, y], 1)
